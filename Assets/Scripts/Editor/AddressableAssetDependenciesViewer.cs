...
public class AddressableAssetDependenciesViewer : EditorWindow
{
    public List<AssetReferenceEditorProperty> properties;
    ...
    string[] dependency_group_types = new string[3] { "Group By Bundles", "Group By Asset References", "No Grouping" };
    ...
    [MenuItem("Window/Custom Tools/Addressable Dependencies/AddressableAssetDepenciesViewer")]
    public static void ShowWindow() {
        //Show existing window instance. If one doesn't exist, make one.
        EditorWindow.GetWindow(typeof(AddressableAssetDependenciesViewer));
    }
    ...
    private void InitializePanels() {
        properties = new List<AssetReferenceEditorProperty>();
        properties.Add(AssetReferenceEditorProperty.CreateInstance());
    ...
    void OnGUI() {
        ...
        for(int i = 0; i < properties.Count; i++) {
            AssetReferenceEditorProperty property = properties[i];
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.PropertyField(property.GetProperty(), false);
            if(i > 0) {
                if(GUILayout.Button("-", GUILayout.MaxWidth(25f))) {
        ...
        UpdateAssetReferenceProperties();
        ...
        if(dependencies.Count > 0) {
            ...
            if(selected_dependency_group_type == 0) {
                dependencies.GroupBy(x => x.bundle_name,(key, g)=> new { bundle_name = key, asset_references = g.Select(s=>s.asset_reference).ToList() })
                    .ToList()
                ...
                dependencies.GroupBy(x => x.asset_reference, (key, g) => new { bundle_names = g.Select(s=>s.bundle_name).ToList(), asset_reference = key })
                    .ToList()
                ...
                for(int i = 0; i < dependencies.Count; i++) {
                    ...
                    EditorGUILayout.LabelField(dependencies[i].asset_reference, EditorStyles.wordWrappedLabel, GUILayout.Width(width / 2));
                    EditorGUILayout.EndHorizontal();
                }
            }
    ...
    private void UpdateAssetReferenceProperties() {
        for(int i = 0; i < properties.Count; i++) {
            properties[i].UpdateProperty();
        }
    }
    ...
    private void RemoveProperty(AssetReferenceEditorProperty property) {
    ...
    private async void UpdateAllDependencies() {
        dependencies = new List<AssetBundleDependency>();
        for(int i = 0; i < properties.Count; i++) {
            List<AssetBundleDependency> current_dependencies = await GetDependencies(properties[i].GetAssetReference());
            dependencies.AddRange(current_dependencies);
        }
        dependencies = dependencies.Distinct().ToList();
        Repaint();
    }

    public async Task<List<AssetBundleDependency>> GetDependencies(AssetReference key) {
        var locsOpHandler = Addressables.LoadResourceLocationsAsync(key);
        var locsTask = locsOpHandler.Task;
        EditorUtility.DisplayProgressBar("Loading", "Fetching Asset Bundle Dependencies", 0.5f);
        await locsTask;
        EditorUtility.ClearProgressBar();
        var locs = locsTask.Result;
        List<AssetBundleDependency> assetReferenceDependencies = new List<AssetBundleDependency>();
        if(locs != null) {
            foreach(var loc in locs) {
                List<string> bundle_names = GetDependenciesForKey(loc.PrimaryKey);
                bundle_names.ForEach(x => {
                        assetReferenceDependencies.Add(new AssetBundleDependency() {
                            bundle_name = x,
                            asset_reference = key.ToString().Replace("["+key.AssetGUID+"]","")
                        });;
                    }
                );
            ...
            Debug.LogError("asset reference Location is null");
        }
        return assetReferenceDependencies;
    }

    private  List<string> GetDependenciesForKey(string addressablesKey) {
        ...
        foreach(AddressableAssetGroup addressableGroup in aaSettings.groups) {
            ...
            foreach(AddressableAssetEntry groupEntry in addressableGroup.entries) {
                ...
                if(groupEntry.address == addressablesKey || groupEntry.labels.Contains(addressablesKey)) {
                    ...
                    string bundleFileName = CheckAndGetBundleName(groupEntry);
...
public class AssetReferenceScriptable : ScriptableObject {
    public AssetReference Asset;
    
}

public class AssetReferenceEditorProperty :SerializedObject {

    public AssetReferenceEditorProperty(Object obj) : base(obj) {
    ...
    public AssetReference GetAssetReference() {
        SerializedProperty serializedProperty = GetProperty();
        return (AssetReference) AddressableAssetDependenciesViewer.GetTargetObjectOfProperty(serializedProperty);
    } 

    public SerializedProperty GetProperty() {
        SerializedProperty serializedProperty = this.FindProperty("Asset");
        return serializedProperty;
    }

    public static AssetReferenceEditorProperty CreateInstance() {
        AssetReferenceScriptable soInstance = ScriptableObject.CreateInstance<AssetReferenceScriptable>();
        return new AssetReferenceEditorProperty(soInstance);
